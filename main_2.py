...
def check(element, start_idx):
    return element.text[start_idx:] if element else "Отсутствует информация"
...
def parse_product(url):
    ...
    try:
        ...
        season = check(data.find("div", class_="option-item sezon even"), 0)
...
def save_results(products, filename="sorted_products.txt"):
    with open(filename, 'w', encoding='utf-8') as f:
        for product in products:
            f.write(f"Наименование: {product['name']}\n")
            f.write(f"Страна: {product['country']}\n")
            f.write(f"Артикул: {product['article']}\n")
            f.write(f"Цвет: {product['color']}\n")
            f.write(f"Тип: {product['type']}\n")
            f.write(f"Материал верха: {product['upper_material']}\n")
